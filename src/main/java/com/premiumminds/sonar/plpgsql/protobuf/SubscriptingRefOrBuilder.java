// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: pg_query.proto

package com.premiumminds.sonar.plpgsql.protobuf;

public interface SubscriptingRefOrBuilder extends
    // @@protoc_insertion_point(interface_extends:pg_query.SubscriptingRef)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <code>.pg_query.Node xpr = 1 [json_name = "xpr"];</code>
   * @return Whether the xpr field is set.
   */
  boolean hasXpr();
  /**
   * <code>.pg_query.Node xpr = 1 [json_name = "xpr"];</code>
   * @return The xpr.
   */
  com.premiumminds.sonar.plpgsql.protobuf.Node getXpr();
  /**
   * <code>.pg_query.Node xpr = 1 [json_name = "xpr"];</code>
   */
  com.premiumminds.sonar.plpgsql.protobuf.NodeOrBuilder getXprOrBuilder();

  /**
   * <code>uint32 refcontainertype = 2 [json_name = "refcontainertype"];</code>
   * @return The refcontainertype.
   */
  int getRefcontainertype();

  /**
   * <code>uint32 refelemtype = 3 [json_name = "refelemtype"];</code>
   * @return The refelemtype.
   */
  int getRefelemtype();

  /**
   * <code>int32 reftypmod = 4 [json_name = "reftypmod"];</code>
   * @return The reftypmod.
   */
  int getReftypmod();

  /**
   * <code>uint32 refcollid = 5 [json_name = "refcollid"];</code>
   * @return The refcollid.
   */
  int getRefcollid();

  /**
   * <code>repeated .pg_query.Node refupperindexpr = 6 [json_name = "refupperindexpr"];</code>
   */
  java.util.List<com.premiumminds.sonar.plpgsql.protobuf.Node> 
      getRefupperindexprList();
  /**
   * <code>repeated .pg_query.Node refupperindexpr = 6 [json_name = "refupperindexpr"];</code>
   */
  com.premiumminds.sonar.plpgsql.protobuf.Node getRefupperindexpr(int index);
  /**
   * <code>repeated .pg_query.Node refupperindexpr = 6 [json_name = "refupperindexpr"];</code>
   */
  int getRefupperindexprCount();
  /**
   * <code>repeated .pg_query.Node refupperindexpr = 6 [json_name = "refupperindexpr"];</code>
   */
  java.util.List<? extends com.premiumminds.sonar.plpgsql.protobuf.NodeOrBuilder> 
      getRefupperindexprOrBuilderList();
  /**
   * <code>repeated .pg_query.Node refupperindexpr = 6 [json_name = "refupperindexpr"];</code>
   */
  com.premiumminds.sonar.plpgsql.protobuf.NodeOrBuilder getRefupperindexprOrBuilder(
      int index);

  /**
   * <code>repeated .pg_query.Node reflowerindexpr = 7 [json_name = "reflowerindexpr"];</code>
   */
  java.util.List<com.premiumminds.sonar.plpgsql.protobuf.Node> 
      getReflowerindexprList();
  /**
   * <code>repeated .pg_query.Node reflowerindexpr = 7 [json_name = "reflowerindexpr"];</code>
   */
  com.premiumminds.sonar.plpgsql.protobuf.Node getReflowerindexpr(int index);
  /**
   * <code>repeated .pg_query.Node reflowerindexpr = 7 [json_name = "reflowerindexpr"];</code>
   */
  int getReflowerindexprCount();
  /**
   * <code>repeated .pg_query.Node reflowerindexpr = 7 [json_name = "reflowerindexpr"];</code>
   */
  java.util.List<? extends com.premiumminds.sonar.plpgsql.protobuf.NodeOrBuilder> 
      getReflowerindexprOrBuilderList();
  /**
   * <code>repeated .pg_query.Node reflowerindexpr = 7 [json_name = "reflowerindexpr"];</code>
   */
  com.premiumminds.sonar.plpgsql.protobuf.NodeOrBuilder getReflowerindexprOrBuilder(
      int index);

  /**
   * <code>.pg_query.Node refexpr = 8 [json_name = "refexpr"];</code>
   * @return Whether the refexpr field is set.
   */
  boolean hasRefexpr();
  /**
   * <code>.pg_query.Node refexpr = 8 [json_name = "refexpr"];</code>
   * @return The refexpr.
   */
  com.premiumminds.sonar.plpgsql.protobuf.Node getRefexpr();
  /**
   * <code>.pg_query.Node refexpr = 8 [json_name = "refexpr"];</code>
   */
  com.premiumminds.sonar.plpgsql.protobuf.NodeOrBuilder getRefexprOrBuilder();

  /**
   * <code>.pg_query.Node refassgnexpr = 9 [json_name = "refassgnexpr"];</code>
   * @return Whether the refassgnexpr field is set.
   */
  boolean hasRefassgnexpr();
  /**
   * <code>.pg_query.Node refassgnexpr = 9 [json_name = "refassgnexpr"];</code>
   * @return The refassgnexpr.
   */
  com.premiumminds.sonar.plpgsql.protobuf.Node getRefassgnexpr();
  /**
   * <code>.pg_query.Node refassgnexpr = 9 [json_name = "refassgnexpr"];</code>
   */
  com.premiumminds.sonar.plpgsql.protobuf.NodeOrBuilder getRefassgnexprOrBuilder();
}
